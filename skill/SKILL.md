...
**⚠️ 两阶段工作流(用户明确要求,防返工)**:做「基于用户文章/用户主题」的素材时,**先出文案草稿**(公众号文章或视频文案)→ 等用户修改定档 → **用户说定档后再出视频/封面/配音等素材**。不要一次全做完等修改;用户会自己改文案,改了再出素材才是定稿内容。涉及用户本人时(公众号文章、迭代复盘文章)尤其如此。

**⚠️ 配音优先的时长预算(必须先定文案,再定幕时长)**:
- 每幕先写**配音文案**(念白,非画面文字),按**正常语速 4.5 字/秒**(实测:无标签默认 ≈4.5 字/秒;`(语速慢)` 标签 ≈3.4 字/秒,仅用于特别舒缓场景)
- **视频总时长 = 文案总字数/4.5 + 4s**(0.8s 开头延迟 + 3s 结尾留白);每幕时长 = 该幕字数/4.5 + 1s
- 示例:113 字 → 25.1s 配音 → 视频 29-30s(做 40s 会拖沓,3.5 字/秒预算同理太慢)
- 画面文字(标题/金句)的停留时间 ≥ 配音念到该处的时间;关键句先出现、配音后到
- 文案与画面文字不必逐字一致:画面可以精简,念白可以补充
- **改时长时直接重写完整 JS 时间轴**(逐条 replace 时间数字极易把 `tl.from(选择器, 参数, 时间)` 拆坏成 `tl.from(数字)`,node --check 验语法 + npm run check 验运行时)
...
- **小红书封面固定规范(用户偏好,强制)**:① 天青色宋瓷风——月白底 #F6F4EF + 天青 #7FA8B5 + 深青 #2E5563,器物剪影/开片/圆点阵意象;② 左上角写**用户自己的小红书 ID**(流程开始时先向用户索取,如 @xxx,深青宋体;未提供则留占位并提醒);③ 右下角写**话题标签,由 AI 根据帖子内容自动生成** 3-5 个(主题词+关键词+人群词,如 #文章一键成片 #天青 #宋瓷),每帖按内容定、不套用固定标签;④ 钩子文字 ≤14 字断两行、最大最醒目
...
**发布到第三方 skill 市场(RedSkill/小红书)**:打包 zip 前必须做**平台名泛化替换**(抖音→短视频平台、微信→图文平台、GitHub→开源社区、品牌名→泛称),发布被拒首因就是自家平台市场里出现竞品平台名。完整流程/替换表/打包验证见 `references/redskill-publishing.md`。
...
**要点**:
- **时长预算先行**(§2):配音文案确定后,核对每幕时长 ≥ 该段文案字数/4.5 + 1s,不足则加长对应幕或删文案——画面要等声音,不能声音追画面
- **文音画同步(剪映式,首选)**:用 `scripts/voice-sync.py` **按幕分段配音**——每幕单独 TTS 调用,拿「实测配音时长」,画面时长 = 配音时长 + 1s(画面先出、配音后到),视频总时长 = Σ(幕配音 + 1s) + 开头 0.4s。**以真实配音为准,不估算语速**,误差 ±0.1s;合成时每段配音按 `voice_delay` 用 adelay 对齐
- **⚠️ 分段配音音色一致性坑(用户实测反馈「有的像有的不像」)**:voiceclone **每次独立调用都会重新提取音色特征**,短文本下模型自由发挥空间大,6 段里有的段"不像本人"。**默认改整段一次生成**(音色 100% 一致,实测 4.5-4.6字/秒),画面按**字数比例**分配幕时长(TTS 语速均匀,±0.3s 误差),合成整段 adelay 对齐;**分段方案仅当必须逐幕精确同步(±0.1s)时用,每段生成后必须逐段试听核对音色**,不一致的段重生成或降级整段
- **⚠️ 分段配音语速坑**:同一文案**分段调用比整段调用明显慢**——实测整段 ≈4.6 字/秒,分段仅 2-3 字/秒(句尾停顿占比大),加 `(语速正常)` 标签可改善到 3-4 字/秒。分段方案以实测时长为准即可(画面跟随,不影响同步),但每幕 +1s 先出 × 6 幕会使视频偏长;要紧凑就用整段配音 + 按字数占比分配幕时长(近似同步)
- **多段合成命令**(每段一个输入,`adelay=毫秒:all=1` + `amix inputs=N:normalize=0`):
  ```bash
  ffmpeg -i 视频.mp4 -i s1.mp3 -i s2.mp3 -i s3.mp3 ... -filter_complex \
    "[1:a]adelay=1400:all=1[a1];[2:a]adelay=9300:all=1[a2];...[a1][a2]...amix=inputs=6:normalize=0[a]" \
    -map 0:v -map "[a]" -c:v copy -c:a aac -shortest out.mp4
  ```
  延迟毫秒 = 该幕画面起点 + 1000(画面先出 1s)
- **同步验证(能量检测)**:合成后逐段检查音频能量——每幕配音起点处 `volumedetect` mean_volume ≈ -15~-20dB,幕间切换处 ≈ -50dB 以下(静音)。**`-ss N -t D` 必须放在 `-i` 前作独立 ffmpeg 命令**,不要写进 Python 管道(会静音误判成 -91dB)
- 风格指令按文章类型调:文化类「沉稳温润」、情感类「娓娓道来」、观点类「笃定有力」;**不要用「语速慢」标签**(实测 3.4 字/秒过慢,正常语速 4.5 字/秒);语速控制靠文案字数
- Python 调用注意:系统 Python SSL 证书常坏(CERTIFICATE_VERIFY_FAILED),脚本内部用 curl 子进程
...
1b. **抽帧验证必须避开动画进行中的时间点**:入场动画(如列表 stagger 从 26.2s 开始)期间元素还是 opacity 0,此时抽帧会误判"内容被裁切/缺失"。验证选**动画完成后的时间点**(如 stagger 结束后 +0.5s,实测 S5 列表 26.2s 入场、28.5s 才是完成态,误在 26.0s 抽帧会看到"列表被裁"的假象),或用行分布检测确认内容边界没触到画布边缘(顶/底裁切 = 内容行到达 y<20 或 y>H-20)
...
7. **公众号抓取**:需要移动端 UA;解析 `<div id="js_content">`;被风控就让用户贴原文
8. **HyperFrames 项目**:`HYPERFRAMES_SKIP_SKILLS=1` 跳过 GitHub skills 检查(网络慢);`render --quality high` 是交付标准,draft 仅迭代;不同比例 = 不同 composition 文件(尺寸编译期锁定)
9. **对比度**:check 的 contrast 采样会落在颜色过渡帧上导致假警告,静态状态达标即可;浅底深字/深底浅字是换肤类动画的刚需。**图形内白字(印章/圆卡)必须用深色渐变底**:浅天青 `#7FA8B5` 上白字只有 ~2.1:1 不达标,用深青 `#2E5563` 系渐变(#3D6472→#2E5563→#264A54)白字才 3:1(宋瓷印章/「瓷」圆卡/消息气泡均如此)
10. **演示类轮播要覆盖全部选项(含初始态)**:换肤/主题轮播别跳过"初始主题"(如初始就是月白,仍要单独演示一次)——用户会注意到"为什么跳过那个",6 个釉色就要演示 6 次
11. **程序化生成多比例**:`width="N"` 属性正则误伤 `data-width`(root 被缩成 1080),用负向后行断言;且**3:4 必须"宽度保持+高度压缩"而非等比缩放**,否则内容变小观感像裁切——用 `scripts/scale-to-34.py`,见 §4
12. **不编造时间线/过程(用户纠正过)**:文案与视频里的迭代叙事必须真实——写过"3 天迭代"被用户否掉("为什么要成三天实际上不是的")。写真实过程:如"16s demo → 10w+ 文章完整成片 → 封面工厂 → 动效升级 → 多平台",不编天数;标签用"第一步/第二步"而非"第 1 天/第 2 天"
13. **本地浏览器正常 ≠ 渲染器正常(CDP 诊断)**:页面在本地 Chrome 打开正常、HyperFrames 渲染输出异常(字号/布局不同)时,别猜——用 headless Chrome CDP 读取实际 computed 值对比:`Chrome --headless=new --remote-debugging-port=9333 <file>` 后台起,node 连 CDP 执行 `Runtime.evaluate` 读 `getComputedStyle(el).fontSize`/`scrollWidth`/`clientWidth`/`parentElement.clientWidth`(脚本: `scripts/fit-diag.mjs`,输出所有 .nowrap 元素的实测值)。实测案例:同一页面本地 fit 后 102px、渲染器实际 ~62px——scrollWidth 字体度量在渲染环境漂移,所以 fit 必须用与字体无关的字符计算法(见 §4)
